...
public class PlayerMinerBase extends Touchable{
	...
	protected float mineEfficiency;
	protected int mineAmount = 1;
	protected float laserAlphaScalar = 0.01f;
	protected float laserWidthScalar = 0.05f;
	protected float laserAlphaMin = 0.3f;
	protected int laserWidthMax = 4;
	
	protected Line laser;
	
	protected Touchable target;
	
	public PlayerMinerBase(float x, float y){
		
		type = Type.FRIENDLY_MINER;
		...
		laser = new Line(0,0,0,0,activity.vertexBufferObjectManager);
		laser.setVisible(false);
		laser.setColor(0.3f, 0.5f, 1.0f);
		...
		updateHandler = new IUpdateHandler() {
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				if (state == State.ATTACKING && target.zeroHealth == false){
					...
					laser.setPosition(gunX, gunY, target.getX(), target.getY());
					
					if (MainActivity.getDistance2(getX(), getY(), target.getX(), target.getY()) >= aggroRange2){
						laser.setVisible(false);
					}

					else if (bulletTimer <= 0 && MainActivity.getDistance2(getX(), getY(), target.getX(), target.getY()) < aggroRange2){
						
						shoot(gunX, gunY, target, toTarget.nor());
						
						laser.setVisible(true);
						
						if (laser.getAlpha() >= 0.8f && laserAlphaScalar > 0){
							laserAlphaScalar *= -1;
						}
						else if (laser.getAlpha() <= laserAlphaMin && laserAlphaScalar < 0){
							laserAlphaScalar *= -1;
						}
						if (laser.getLineWidth() >= laserWidthMax && laserWidthScalar > 0){
							laserWidthScalar *= -1;
						}
						else if (laser.getLineWidth() <= 1 && laserWidthScalar < 0){
							laserWidthScalar *= -1;
						}
						
						laser.setZIndex(1000);
						laser.setAlpha(laser.getAlpha() + laserAlphaScalar);
						laser.setLineWidth(laser.getLineWidth() + laserWidthScalar);
					}
	...
	public void findTarget(){
		...
		for (int i = 0; i < scene.touchableListAsteroid.size(); i++){
			if (MainActivity.getDistance2(getX(), getY(), scene.touchableListAsteroid.get(i).getX(), scene.touchableListAsteroid.get(i).getY()) < distanceToClosest2){
				distanceToClosest2 = MainActivity.getDistance2(getX(), getY(), scene.touchableListAsteroid.get(i).getX(), scene.touchableListAsteroid.get(i).getY());
				indexOfClosest = i;
				foundTarget = true;
			}
		}
		...
		if (foundTarget){
			target = scene.touchableListAsteroid.get(indexOfClosest);
